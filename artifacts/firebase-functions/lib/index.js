...
async function getSessionUser(req) {
    ...
    const result = await query(`SELECT u.* FROM users u
...
app.use((0, cors_1.default)({ origin: true, credentials: true, methods: ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"] }));
...
app.get("/api/app/version", async (_req, res) => {
    ...
    res.json({ version: ((_a = r.rows[0]) === null || _a === void 0 ? void 0 : _a.value) || "2.5.5", force_update: false });
});
// ══ Feature Flags ══
app.get("/api/app/feature-flags", async (_req, res) => {
    ...
    res.json({
        ...
        ride_status: map.ride_status || "soon",
    });
});
...
app.post("/api/auth/register", async (req, res) => {
    try {
        const { name, phone, email, password, national_id, birth_date, neighborhood, gender } = req.body;
        if (!name || !password || (!phone && !email))
            return res.status(400).json({ error: "البيانات ناقصة" });
        if (password.length < 6)
            return res.status(400).json({ error: "كلمة المرور يجب أن تكون 6 أحرف على الأقل" });
        const hash = await bcryptjs_1.default.hash(password, 10);
        const result = await query(`INSERT INTO users (name, phone, email, password_hash, national_id, birth_date, neighborhood, gender)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`, [name, phone || null, email || null, hash, national_id || null, birth_date || null, neighborhood || null, gender || null]);
        const user = result.rows[0];
        const token = (0, crypto_1.randomBytes)(32).toString("hex");
        await query(`INSERT INTO user_sessions (user_id, token) VALUES ($1,$2)`, [user.id, token]);
        return res.json({ user: safeUserPayload(user), token });
        ...
        const msg = (err === null || err === void 0 ? void 0 : err.code) === "23505" ? "رقم الهاتف أو البريد مسجل مسبقاً" : "فشل إنشاء الحساب";
...
app.post("/api/ads", async (req, res) => {
    try {
        const user = await getSessionUser(req);
        const { institution_name, title, description, type, contact_phone, contact_email, website_url } = req.body;
        if (!institution_name || !title)
            return res.status(400).json({ error: "البيانات ناقصة" });
        const result = await query(`INSERT INTO ads (institution_name, title, description, type, contact_phone, contact_email, website_url, user_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`, [institution_name, title, description || null, type || "banner", contact_phone || null, contact_email || null, website_url || null, (user === null || user === void 0 ? void 0 : user.id) || null]);
...
app.patch("/api/admin/ads/:id/status", async (req, res) => {
    ...
    if (!me || !["admin", "moderator"].includes(me.role))
        return res.status(403).json({ error: "غير مصرح" });
    const { status, priority, expires_days } = req.body;
    let expires_at = null;
    if (expires_days) {
        const d = new Date();
...
app.post("/api/social/posts", async (req, res) => {
    try {
        ...
        const { content, category = "عام", is_anonymous = false, image_url } = req.body;
        if (!content)
            return res.status(400).json({ error: "المحتوى مطلوب" });
        const result = await query(`INSERT INTO social_posts (author_id, author_name, content, category, is_anonymous, image_url) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [user.id, user.name, content, category, is_anonymous, image_url || null]);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        console.error(err);
...
app.post("/api/social/posts/:id/comments", async (req, res) => {
    try {
        ...
        if (!content)
            return res.status(400).json({ error: "المحتوى مطلوب" });
        const result = await query(`INSERT INTO social_comments (post_id, author_id, author_name, content) VALUES ($1,$2,$3,$4) RETURNING *`, [req.params.id, user.id, user.name, content]);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        console.error(err);
...
app.post("/api/jobs", async (req, res) => {
    try {
        ...
        if (!title || !description)
            return res.status(400).json({ error: "البيانات ناقصة" });
        const result = await query(`INSERT INTO jobs (title, company, type, location, description, contact_phone, salary, user_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`, [title, company || null, type || "fulltime", location || null, description, contact_phone || null, salary || null, (user === null || user === void 0 ? void 0 : user.id) || null]);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        console.error(err);
...
app.post("/api/market", async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO merchant_spaces (name, description, category, phone, address, user_id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [name, description || null, category || null, phone || null, address || null, (user === null || user === void 0 ? void 0 : user.id) || null]);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        console.error(err);
...
app.post("/api/lost", async (req, res) => {
    try {
        ...
        if (!title || !type)
            return res.status(400).json({ error: "البيانات ناقصة" });
        const result = await query(`INSERT INTO lost_items (type, title, description, location, contact_phone, user_id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [type, title, description || null, location || null, contact_phone || null, (user === null || user === void 0 ? void 0 : user.id) || null]);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        console.error(err);
...
app.post("/api/admin/specialists", async (req, res) => {
    ...
    if (!me || !["admin", "moderator"].includes(me.role))
        return res.status(403).json({ error: "غير مصرح" });
    const { name, specialty, clinic_name, phone, address, schedule, image_url, is_active, order_index } = req.body;
    const r = await query(`INSERT INTO specialists (name, specialty, clinic_name, phone, address, schedule, image_url, is_active, order_index) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`, [name, specialty, clinic_name || null, phone || null, address || null, schedule || null, image_url || null, is_active !== null && is_active !== void 0 ? is_active : true, order_index !== null && order_index !== void 0 ? order_index : 0]);
...
app.patch("/api/admin/specialists/:id", async (req, res) => {
    ...
    if (!me || !["admin", "moderator"].includes(me.role))
        return res.status(403).json({ error: "غير مصرح" });
    const { name, specialty, clinic_name, phone, address, schedule, image_url, is_active, order_index } = req.body;
    await query(`UPDATE specialists SET name=COALESCE($1,name), specialty=COALESCE($2,specialty), clinic_name=COALESCE($3,clinic_name), phone=COALESCE($4,phone), address=COALESCE($5,address), schedule=COALESCE($6,schedule), image_url=COALESCE($7,image_url), is_active=COALESCE($8,is_active), order_index=COALESCE($9,order_index) WHERE id=$10`, [name || null, specialty || null, clinic_name || null, phone || null, address || null, schedule || null, image_url || null, is_active !== null && is_active !== void 0 ? is_active : null, order_index !== null && order_index !== void 0 ? order_index : null, req.params.id]);
    res.json({ ok: true });
});
...
app.post("/api/push-token", async (req, res) => {
    try {
        ...
        await query(`INSERT INTO push_tokens (user_id, token, platform) VALUES ($1,$2,$3) ON CONFLICT (token) DO UPDATE SET user_id=$1, platform=$3`, [(user === null || user === void 0 ? void 0 : user.id) || null, token, platform]);
        res.json({ ok: true });
    }
    catch (err) {
        console.error(err);
...
app.get("/api/admin/users", async (req, res) => {
    ...
    if (search) {
        params.push(`%${search}%`);
        q += ` AND (name ILIKE $${params.length} OR phone ILIKE $${params.length} OR email ILIKE $${params.length})`;
    }
    if (role) {
        params.push(role);
...
app.post("/api/auth/register-admin", async (req, res) => {
    ...
    try {
        ...
        const pin = ((_a = settingResult.rows[0]) === null || _a === void 0 ? void 0 : _a.value) || "4444";
        if (admin_code !== pin)
            return res.status(403).json({ error: "رمز المشرف غير صحيح" });
        const hash = await bcryptjs_1.default.hash(password, 10);
        const result = await query(`INSERT INTO users (name, email, password_hash, role) VALUES ($1,$2,$3,'admin') RETURNING id, name, role`, [name, email, hash]);
        res.json({ user: result.rows[0] });
    }
    catch (_b) {
        res.status(400).json({ error: "البريد مسجل مسبقاً" });
    }
});
// ══ fallback 404 ══
