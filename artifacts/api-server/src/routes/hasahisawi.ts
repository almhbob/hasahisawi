...
export async function initHasahisawiDb() {
  ...
  `);
  // [FIX-5] ترحيل: إزالة القيد القديم UNIQUE(user_id) الذي يمنع المستخدم من تسجيل أكثر من جهاز
  // ثم إضافة قيد UNIQUE(user_id, token) الصحيح
  await query(`
    DO $$ BEGIN
      IF EXISTS (
        ...
        ALTER TABLE push_tokens DROP CONSTRAINT push_tokens_user_id_key;
      END IF;
    END $$
  `).catch(() => {});
  await query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_push_tokens_user_token ON push_tokens(user_id, token)`);
...
async function getSessionUser(req: Request): Promise<Record<string, unknown> | null> {
  if (!pool) return null; // [FIX-1] لا تكسر الخادم إذا لم تكن قاعدة البيانات مُضبوطة
  const auth = req.headers.authorization;
  if (!auth || !auth.startsWith("Bearer ")) return null;
  const token = auth.slice(7);
  if (!token || token.length < 32) return null; // [FIX-1] رفض tokens قصيرة/فارغة
  try {
    ...
    return null; // [FIX-1] أي خطأ في DB لا يوقف الطلب
  }
}
...
async function isAdminRequest(req: Request): Promise<boolean> {
  ...
  if (submittedPin && submittedPin.length >= 4 && submittedPin.length <= 20) {
    if (!pool) return safeCompare(submittedPin, DEFAULT_ADMIN_PIN); // [FIX-6] guard pool null
    try {
      ...
      return false; // [FIX-6] DB error → deny access safely
    }
  }
  return false;
}
...
router.post("/push-tokens", async (req: Request, res: Response) => {
  try {
    ...
    // [FIX-5] upsert باستخدام (user_id, token) — يدعم تسجيل أجهزة متعددة للمستخدم الواحد
    await query(
...
router.get("/admin/dashboard-stats", async (req: Request, res: Response) => {
  res.setHeader("Content-Type", "application/json"); // [FIX-B] ضمان JSON دائماً
  try {
    const user = await getSessionUser(req);
    if (!user || (user.role !== "admin" && user.role !== "moderator")) {
      return res.status(403).json({ error: "غير مصرح" });
  ...
  try {
    ...
    for (const apiKey of apiKeys) {
      for (const model of MODELS) {
        // [FIX-2] timeout 20s — بدونه الطلب يتعلق إلى الأبد ويحجب worker thread
        const geminiCtrl = new AbortController();
        const geminiTimeout = setTimeout(() => geminiCtrl.abort(), 20_000);
        let geminiRes: Response;
        try {
          geminiRes = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`,
            { method: "POST", headers: { "Content-Type": "application/json" }, body: requestBody, signal: geminiCtrl.signal }
          );
          clearTimeout(geminiTimeout);
        } catch (fetchErr: any) {
          clearTimeout(geminiTimeout);
          if (fetchErr?.name === "AbortError") { lastStatus = 504; console.warn(`[AI] Gemini timeout (${model})`); break; }
          lastStatus = 503; break;
        }
...
router.post("/institution-applications", async (req: Request, res: Response) => {
  res.setHeader("Content-Type", "application/json"); // [FIX-D] ضمان JSON دائماً
  try {
    const {
...
// ════════════════════════════════════════════════════════════════
setImmediate(() => (async () => {
  // [FIX-3] try/catch مضافة — بدونها unhandled rejection يوقف الخادم
  try {
    await query(`
...
})().catch(e => console.error("events init outer:", e)));
...
router.post("/lawyer-applications", async (req: Request, res: Response) => {
  res.setHeader("Content-Type", "application/json"); // [FIX-C] ضمان JSON دائماً
  try {
    const b = req.body || {};
