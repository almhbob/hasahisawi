...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS competitions (
      ...
      created_by VARCHAR(100) NOT NULL DEFAULT 'الإدارة',
      image_url TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )
  `);
  await query(`
    CREATE TABLE IF NOT EXISTS competition_submissions (
      ...
      user_id VARCHAR(100),
      user_name VARCHAR(100) NOT NULL DEFAULT 'مشارك',
      content TEXT NOT NULL,
      score INTEGER,
      rank INTEGER,
      status VARCHAR(20) NOT NULL DEFAULT 'pending',
  ...
  if (parseInt(compCount.rows[0].count) === 0) {
    await query(`
      INSERT INTO competitions (title, description, category, prize, deadline, status, participants_count, created_by) VALUES
      ('مسابقة أفضل مقال علمي', 'اكتب مقالاً علمياً متميزاً في أي مجال معرفي تختاره', 'academic', 'جائزة نقدية 500 ألف', NOW() + INTERVAL '30 days', 'open', 12, 'الإدارة'),
      ('تحدي التصوير الفوتوغرافي', 'التقط صورة تعبّر عن الحياة اليومية في مدينة الحصاحيصا', 'creative', 'كاميرا احترافية', NOW() + INTERVAL '15 days', 'open', 8, 'الإدارة'),
      ('مسابقة البرمجة الإبداعية', 'طوّر تطبيقاً مفيداً للمجتمع المحلي', 'technology', 'لابتوب + شهادة', NOW() + INTERVAL '45 days', 'open', 5, 'الإدارة'),
      ('مسابقة الشعر العربي', 'قصيدة في رثاء الوطن أو مدح العلم والعلماء', 'cultural', 'درع + شهادة تقدير', NOW() - INTERVAL '5 days', 'closed', 23, 'الإدارة')
    `);
  }

  app.get("/api/competitions", async (_req: Request, res: Response) => {
    try {
      const r = await query("SELECT * FROM competitions ORDER BY created_at DESC");
      res.json(r.rows);
    } catch (err) { res.status(500).json({ error: "Server error" }); }
  });
  ...
  app.post("/api/competitions/:id/submit", async (req: Request, res: Response) => {
    ...
    try {
      const comp = await query("SELECT * FROM competitions WHERE id=$1", [req.params.id]);
      if (!comp.rows[0]) return res.status(404).json({ error: "المسابقة غير موجودة" });
      if (comp.rows[0].status !== "open") return res.status(400).json({ error: "المسابقة مغلقة" });
      const user = await query("SELECT name, display_name FROM users WHERE firebase_uid=$1", [uid]);
      const uname = user.rows[0]?.display_name || user.rows[0]?.name || "مشارك";
      await query(
        "INSERT INTO competition_submissions (competition_id, user_id, user_name, content) VALUES ($1,$2,$3,$4)",
        [req.params.id, uid, uname, content]
      );
      await query("UPDATE competitions SET participants_count = participants_count + 1 WHERE id=$1", [req.params.id]);
      res.json({ success: true });
    } catch (err) { res.status(500).json({ error: "Server error" }); }
  });

  app.get("/api/competitions/leaderboard", async (_req: Request, res: Response) => {
    try {
      const r = await query(`
        SELECT cs.*, c.title AS competition_title
        FROM competition_submissions cs
        JOIN competitions c ON c.id = cs.competition_id
        WHERE cs.status IN ('reviewed','winner') AND cs.score IS NOT NULL
        ORDER BY cs.score DESC LIMIT 50
      `);
      res.json(r.rows);
    } catch (err) { res.status(500).json({ error: "Server error" }); }
  });

  app.get("/api/competitions/my-submissions", async (req: Request, res: Response) => {
    const uid = (req as any).userId;
    if (!uid) return res.status(401).json({ error: "Unauthorized" });
    try {
      const r = await query(`
