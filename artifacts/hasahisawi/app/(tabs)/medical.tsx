...
// قسم الدليل الطبي
// ══════════════════════════════════════════════════════
function DirectoryTab({ search, filter, facilities, t, isRTL, tr, dbClinics, onClinicPress }: any) {
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={[styles.listContent, { paddingBottom: Platform.OS === "web" ? 100 : 120 }]} showsVerticalScrollIndicator={false}>
      ...
      {/* ── قسم المستوصفات المسجّلة في المنصة ── */}
      {dbClinics && dbClinics.length > 0 && (
        <Animated.View entering={FadeInDown.delay(30).springify()}>
          <View style={clnSty.section}>
            <View style={clnSty.sectionHeader}>
              <View style={clnSty.headerLine} />
              <MaterialCommunityIcons name="shield-check" size={15} color="#E74C6F" />
              <Text style={clnSty.sectionTitle}>مستوصفات حصاحيصاوي المسجّلة</Text>
            </View>
            {dbClinics.map((c: any, idx: number) => (
              <Animated.View key={c.id} entering={FadeInDown.delay(60 + idx * 50).springify()}>
                <TouchableOpacity
                  style={clnSty.card}
                  onPress={() => onClinicPress(c.id)}
                  activeOpacity={0.85}
                >
                  <View style={clnSty.cardLeft}>
                    <Ionicons name="chevron-back" size={18} color="#E74C6F60" />
                  </View>
                  <View style={{ flex: 1, alignItems: "flex-end", gap: 4 }}>
                    <View style={clnSty.cardNameRow}>
                      <View style={clnSty.verifiedBadge}>
                        <MaterialCommunityIcons name="shield-check" size={11} color="#E74C6F" />
                        <Text style={clnSty.verifiedText}>موثّق</Text>
                      </View>
                      <Text style={clnSty.cardName} numberOfLines={1}>{c.inst_name}</Text>
                    </View>
                    {c.inst_description ? (
                      <Text style={clnSty.cardDesc} numberOfLines={2}>{c.inst_description}</Text>
                    ) : null}
                    <View style={clnSty.cardFooter}>
                      {c.inst_phone ? (
                        <View style={clnSty.footerItem}>
                          <Ionicons name="call-outline" size={12} color={Colors.textMuted} />
                          <Text style={clnSty.footerText}>{c.inst_phone}</Text>
                        </View>
                      ) : null}
                      {c.inst_address ? (
                        <View style={clnSty.footerItem}>
                          <Ionicons name="location-outline" size={12} color={Colors.textMuted} />
                          <Text style={clnSty.footerText} numberOfLines={1}>{c.inst_address}</Text>
                        </View>
                      ) : null}
                    </View>
                  </View>
                  <View style={clnSty.iconBox}>
      ...
      {filtered.length === 0 && dbClinics?.length === 0 && (
...
export default function MedicalScreen() {
  ...
  const [dbClinics, setDbClinics] = useState<any[]>([]);
  const router = useRouter();
  ...
  const loadClinics = async () => {
    try {
      const base = getApiUrl();
      if (!base) return;
      const r = await fetch(`${base}/api/clinics`);
      if (r.ok) { const d = await r.json() as any; setDbClinics(d.clinics || []); }
    } catch {}
  };
  useEffect(() => { load(); loadClinics(); }, []);
  useFocusEffect(useCallback(() => { load(); loadClinics(); }, []));
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {activeTab === "directory"     && <DirectoryTab search={search} filter={filter} facilities={facilities} t={t} isRTL={isRTL} tr={tr} dbClinics={dbClinics} onClinicPress={(id: number) => router.push({ pathname: "/clinic-profile", params: { id: String(id) } } as any)} />}
...
const clnSty = StyleSheet.create({
  section: {
    backgroundColor: Colors.cardBg, borderRadius: 20, padding: 14,
    borderWidth: 1, borderColor: "#E74C6F25",
    gap: 10, marginBottom: 4,
    shadowColor: "#E74C6F", shadowOpacity: 0.08, shadowOffset: { width: 0, height: 4 }, shadowRadius: 12, elevation: 4,
  },
  sectionHeader: {
    flexDirection: "row-reverse", alignItems: "center", gap: 8, marginBottom: 2,
  },
  headerLine: { flex: 1, height: 1, backgroundColor: "#E74C6F25" },
  sectionTitle: { fontFamily: "Cairo_700Bold", fontSize: 14, color: "#E74C6F" },
  card: {
    flexDirection: "row-reverse", alignItems: "center", gap: 10,
    backgroundColor: Colors.bg, borderRadius: 14, padding: 12,
    borderWidth: 1, borderColor: "#E74C6F20",
  },
  iconBox: {
  ...
  cardNameRow: { flexDirection: "row-reverse", alignItems: "center", gap: 6 },
  cardName: { fontFamily: "Cairo_700Bold", fontSize: 15, color: Colors.textPrimary },
  verifiedBadge: {
    flexDirection: "row-reverse", alignItems: "center", gap: 3,
    backgroundColor: "#E74C6F15", borderRadius: 7, paddingHorizontal: 6, paddingVertical: 2,
    borderWidth: 1, borderColor: "#E74C6F30",
  },
  verifiedText: { fontFamily: "Cairo_500Medium", fontSize: 10, color: "#E74C6F" },
  cardDesc: { fontFamily: "Cairo_400Regular", fontSize: 12, color: Colors.textMuted, lineHeight: 19 },
  cardFooter: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 8, marginTop: 2 },
  footerItem: { flexDirection: "row-reverse", alignItems: "center", gap: 4 },
  footerText: { fontFamily: "Cairo_400Regular", fontSize: 11, color: Colors.textMuted },
});
